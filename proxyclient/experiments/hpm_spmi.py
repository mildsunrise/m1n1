...
class HpmSPMI:
    ...
    def wakeup(self):
        ...
        # we just need to verify that the device has woken up,
        # so it's enough to check that it's acting on writes.
        # we don't need to actually select a register, so we use write_reg
        target = self.spmi.read_reg(self.slave, 0) ^ 1
        for i in range(10):
            self.spmi.write_reg(self.slave, 0, target)
            if self.spmi.read_reg(self.slave, 0) == target: return
